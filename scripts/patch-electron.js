// Patches Electron.app so it shows "Helm" with the correct icon in dev mode
...
const destApp = path.join(distDir, 'Helm.app');
...
fs.writeFileSync(path.join(electronDir, 'path.txt'), 'Helm.app/Contents/MacOS/Electron');

// Patch Info.plist
const plist = path.join(destApp, 'Contents', 'Info.plist');
execSync(`/usr/libexec/PlistBuddy -c "Set CFBundleName Helm" "${plist}"`);
execSync(`/usr/libexec/PlistBuddy -c "Set CFBundleDisplayName Helm" "${plist}"`);

// Localized display name
const lprojDir = path.join(destApp, 'Contents', 'Resources', 'en.lproj');
fs.mkdirSync(lprojDir, { recursive: true });
fs.writeFileSync(path.join(lprojDir, 'InfoPlist.strings'), 'CFBundleName = "Helm";\nCFBundleDisplayName = "Helm";\n');
...
console.log('Patched Electron → Helm (bundle rename + plist + icon)');
